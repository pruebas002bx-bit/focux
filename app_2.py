...
CORS(app, origins=[
    'https://focux.netlify.app', # <-- AÑADE ESTA LÍNEA AL INICIO
...
def delete_document(doc_id):
    try:
        with db_lock:
            ...
            cursor.execute("SELECT google_drive_file_id FROM documents WHERE id = ?", (doc_id,))
...
def update_cover(doc_id):
    ...
    try:
        with db_lock:
            ...
            cursor.execute("UPDATE documents SET thumbnail_url = ? WHERE id = ?", (new_url, doc_id))
    ...
    try:
        ...
        for db_name in set(db_names):
            ...
            for board in boards_in_db:
                ...
                cursor.execute("SELECT * FROM notes WHERE board_id = ? AND user_email = ?", (board['id'], user_email))
...
def init_db():
    """Initializes the main database, creating the necessary tables if they do not exist."""
    try:
        with db_lock:
            conn = get_db_connection()
            cursor = conn.cursor()
            
            cursor.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, email TEXT UNIQUE, password TEXT, registration_date TEXT, last_login TEXT, manager_id TEXT, access_expires_on TEXT)')
            cursor.execute('CREATE TABLE IF NOT EXISTS boards (id INTEGER PRIMARY KEY, owner_email TEXT, name TEXT, board_data TEXT, created_date TEXT, updated_date TEXT, category TEXT)')
            cursor.execute('CREATE TABLE IF NOT EXISTS collaborators (board_id INTEGER, user_email TEXT, permission_level TEXT NOT NULL DEFAULT \'editor\', PRIMARY KEY (board_id, user_email), FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY, board_id INTEGER, user_email TEXT, content TEXT, color TEXT, created_date TEXT, updated_date TEXT, FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS assistants (id TEXT PRIMARY KEY, name TEXT, avatar_url TEXT, description TEXT, prompt TEXT, knowledge_base TEXT, is_public INTEGER DEFAULT 0)')
            cursor.execute('CREATE TABLE IF NOT EXISTS board_chats (id INTEGER PRIMARY KEY, board_id INTEGER, user_email TEXT, user_name TEXT, message TEXT, timestamp TEXT, FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS assistant_sharing (assistant_id TEXT, user_email TEXT, PRIMARY KEY (assistant_id, user_email), FOREIGN KEY (assistant_id) REFERENCES assistants(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY, title TEXT, message TEXT, timestamp TEXT, type TEXT, target_info TEXT)')
            cursor.execute('CREATE TABLE IF NOT EXISTS notification_views (notification_id INTEGER, user_email TEXT, PRIMARY KEY (notification_id, user_email), FOREIGN KEY (notification_id) REFERENCES notifications(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS conversations (id TEXT PRIMARY KEY, participants_json TEXT, last_ts TEXT)')
            cursor.execute('CREATE TABLE IF NOT EXISTS direct_messages (id INTEGER PRIMARY KEY, conv_id TEXT, ts TEXT, sender_email TEXT, receiver_email TEXT, text TEXT, is_read INTEGER DEFAULT 0, FOREIGN KEY (conv_id) REFERENCES conversations(id) ON DELETE CASCADE)')
            cursor.execute('CREATE TABLE IF NOT EXISTS stickers (id TEXT PRIMARY KEY, name TEXT, category TEXT, url TEXT NOT NULL)')
            cursor.execute('''CREATE TABLE IF NOT EXISTS focux_messages (
            ...
            # Esta es la tabla que causa el error. Asegúrate de que esta línea esté aquí.
            cursor.execute('''
...
def init_db_for_file(db_path):
    """Initializes the table schema in a specific database file."""
    conn = psycopg2.connect(db_path)
    cursor = conn.cursor()
    # --- INICIO DE LA CORRECCIÓN ---
    # Se eliminan las tablas de asistentes de las bases de datos secundarias.
    # Los asistentes son un recurso global y solo deben existir en la BD principal.
    cursor.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, email TEXT UNIQUE, password TEXT, registration_date TEXT, last_login TEXT, manager_id TEXT, access_expires_on TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS boards (id INTEGER PRIMARY KEY, owner_email TEXT, name TEXT, board_data TEXT, created_date TEXT, updated_date TEXT, category TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS collaborators (board_id INTEGER, user_email TEXT, permission_level TEXT NOT NULL DEFAULT \'editor\', PRIMARY KEY (board_id, user_email), FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
    cursor.execute('CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY, board_id INTEGER, user_email TEXT, content TEXT, color TEXT, created_date TEXT, updated_date TEXT, FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
    cursor.execute('CREATE TABLE IF NOT EXISTS board_chats (id INTEGER PRIMARY KEY, board_id INTEGER, user_email TEXT, user_name TEXT, message TEXT, timestamp TEXT, FOREIGN KEY (board_id) REFERENCES boards(id) ON DELETE CASCADE)')
    # Las tablas 'assistants' y 'assistant_sharing' han sido removidas de aquí intencionalmente.
    cursor.execute('CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY, title TEXT, message TEXT, timestamp TEXT, type TEXT, target_info TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS notification_views (notification_id INTEGER, user_email TEXT, PRIMARY KEY (notification_id, user_email), FOREIGN KEY (notification_id) REFERENCES notifications(id) ON DELETE CASCADE)')
    cursor.execute('CREATE TABLE IF NOT EXISTS conversations (id TEXT PRIMARY KEY, participants_json TEXT, last_ts TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS direct_messages (id INTEGER PRIMARY KEY, conv_id TEXT, ts TEXT, sender_email TEXT, receiver_email TEXT, text TEXT, is_read INTEGER DEFAULT 0, FOREIGN KEY (conv_id) REFERENCES conversations(id) ON DELETE CASCADE)')
    ...
    try:
        with db_lock:
            ...
            cursor.execute("""
                ...
                title=?, content=?, color=?, image_url=?, button_text=?, button_url=?, 
                is_active=?, start_date=?, end_date=?, target_info=?
                WHERE id=?
            """, (
...
def delete_focux_message(message_id):
    ...
    try:
        with db_lock:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM focux_messages WHERE id = ?", (message_id,))
            if cursor.rowcount == 0:
                conn.close()
...
def schedule_telegram_reminder(user_email, card_data):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT chat_id FROM telegram_connections WHERE user_email = ? AND is_active = 1",
            (user_email,)
        )
    ...
    while True:
        try:
            ...
            with db_lock:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT * FROM scheduled_reminders WHERE notification_time <= ? AND sent = 0",
                    (now_utc.isoformat(),)
                )
            ...
            for reminder in reminders_to_send:
                try:
                    ...
                    if result.get('success'):
                        ...
                        with db_lock:
                            cursor.execute("UPDATE scheduled_reminders SET sent = 1 WHERE id = ?", (reminder['id'],))
                            conn.commit()
                    else:
...
def focux_view_delete_card():
    ...
    try:
        with db_lock:
            ...
            cursor.execute("UPDATE boards SET board_data = ? WHERE id = ?", (json.dumps(board_data), board_id))
            conn.commit()
            conn.close()
...
def delete_database_dynamic():
    ...
    try:
        master_conn = psycopg2.connect(MASTER_DB)
        master_cursor = master_conn.cursor()
        master_cursor.execute("DELETE FROM managed_databases WHERE name = ?", (safe_db_name,))
        master_conn.commit()
        master_conn.close()
...
def set_user_expiry():
    ...
    try:
        with db_lock:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET access_expires_on = ? WHERE email = ?", (expiry_date_to_db, email))
            if cursor.rowcount == 0:
                conn.close()
    ...
    try:
        ...
        if home_conn:
            with db_lock:
                ...
                home_cursor.execute("SELECT id FROM boards WHERE owner_email = ?", (email,))
                ...
                home_cursor.execute("DELETE FROM users WHERE email = ?", (email,))
                home_conn.commit()
                home_conn.close()
        ...
        for db_name in set(all_db_names):
            ...
            with db_lock:
                ...
                cursor.execute("DELETE FROM collaborators WHERE user_email = ?", (email,))
                conn.commit()
                conn.close()
        ...
        with db_lock:
            ...
            main_cursor.execute("DELETE FROM telegram_connections WHERE user_email = ?", (email,))
            main_cursor.execute("DELETE FROM direct_messages WHERE sender_email = ? OR receiver_email = ?", (email, email))
            main_cursor.execute("DELETE FROM notification_views WHERE user_email = ?", (email,))
            main_cursor.execute("DELETE FROM assistant_sharing WHERE user_email = ?", (email,))
            main_conn.commit()
            main_conn.close()
...
def get_all_assistants():
    ...
    try:
        ...
        for assistant in assistants_raw:
            cursor.execute("SELECT user_email FROM assistant_sharing WHERE assistant_id = ?", (assistant['id'],))
...
def get_saved_ai_board(board_id):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, board_json, notes_json, created_at FROM ai_generated_boards WHERE id = ?", (board_id,))
        board_row = cursor.fetchone()
        conn.close()
...
def delete_saved_ai_board(board_id):
    ...
    try:
        with db_lock:
            ...
            cursor.execute("SELECT name FROM ai_generated_boards WHERE id = ?", (board_id,))
            ...
            cursor.execute("DELETE FROM ai_generated_boards WHERE id = ?", (board_id,))
            conn.commit()
            conn.close()
        ...
        with db_lock:
            ...
            for assistant in assistants_data:
                ...
                # CASO 2: Es un asistente existente que se está actualizando.
                    ...
                    cursor.execute(
                        "UPDATE assistants SET name=?, avatar_url=?, description=?, prompt=?, knowledge_base=?, is_public=? WHERE id=?",
                        (
                            assistant.get('name'), 
                ...
                cursor.execute("DELETE FROM assistant_sharing WHERE assistant_id = ?", (assistant_id,))
        ...
        if target_mode == 'all':
            ...
            if target_value == 'Principal':
                ...
                cursor.execute("""
                    ...
                    FROM telegram_connections t
                    JOIN users u ON t.user_email = u.email
                    WHERE t.is_active = 1 AND u.manager_id = ?
                """, (target_value,))
            connections = cursor.fetchall()
...
def get_references():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM \"references\" WHERE board_id = ? ORDER BY created_date DESC", (board_id,))
...
def update_reference(ref_id):
    ...
    try:
        ...
        with db_lock:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE "references" SET title = ?, url = ?, updated_date = ? WHERE id = ? AND board_id = ?',
...
def delete_reference(ref_id):
    ...
    try:
        ...
        with db_lock:
            cursor = conn.cursor()
            cursor.execute('DELETE FROM "references" WHERE id = ? AND board_id = ?', (ref_id, board_id))
            if cursor.rowcount == 0:
                conn.close()
        ...
        if result['success']:
            try:
                with db_lock:
                    ...
                    cursor.execute("SELECT id FROM telegram_connections WHERE user_email = ?", (user_email,))
                    existing = cursor.fetchone()
                    
                    if existing:
                        cursor.execute(
                            "UPDATE telegram_connections SET chat_id = ?, connected_at = ?, is_active = 1 WHERE user_email = ?",
...
def get_documents():
    ...
    try:
        ...
        cursor.execute("SELECT * FROM documents WHERE board_id = ? ORDER BY created_date DESC", (board_id,))
...
def register():
    ...
    if manager_id != 'Principal':
        master_conn = psycopg2.connect(MASTER_DB)
        master_cursor = master_conn.cursor()
        master_cursor.execute("SELECT password_hash FROM managed_databases WHERE name = ?", (manager_id,))
...
def login():
    ...
    try:
        ...
        if user:
            ...
            master_conn.row_factory = psycopg2.Row
            master_cursor = master_conn.cursor()
            master_cursor.execute("SELECT logo_url FROM database_settings WHERE name = ?", (manager_id,))
    ...
    try:
        ...
        for db_name in set(all_db_names):
            ...
            for board in boards_in_db:
                ...
                cursor.execute("SELECT user_email, permission_level FROM collaborators WHERE board_id = ?", (board['id'],))
...
def update_board(board_id):
    ...
    try:
        ...
        with db_lock:
            ...
            now = datetime.now(timezone.utc).isoformat()
            cursor.execute(
                "UPDATE boards SET board_data = ?, updated_date = ? WHERE id = ?",
...
def update_collaborator_permission(board_id):
    ...
    try:
        ...
        with db_lock:
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE collaborators SET permission_level = ? 
                WHERE board_id = ? AND user_email = ?
            """, (new_permission, board_id, collaborator_email))
            conn.commit()
            
            cursor.execute("SELECT user_email, permission_level FROM collaborators WHERE board_id = ?", (board_id,))
...
def check_editor_permission(board_id, user_email):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT permission_level FROM collaborators WHERE board_id = ? AND user_email = ?", (board_id, user_email))
...
def share_board(board_id):
    ...
    # --- Lógica para ELIMINAR un colaborador (DELETE) ---
        ...
        try:
            ...
            with db_lock:
                cursor = conn.cursor()
                cursor.execute("DELETE FROM collaborators WHERE board_id = ? AND user_email = ?", (board_id, email_to_remove))
                conn.commit()
...
def update_board_name(board_id):
    ...
    try:
        with db_lock:
            ...
            cursor.execute("SELECT board_id FROM collaborators WHERE board_id = ? AND user_email = ?", (board_id, email))
            if not cursor.fetchone():
                conn.close()
                return jsonify(success=False, message="Acceso denegado"), 403

            cursor.execute("UPDATE boards SET name = ?, updated_date = ? WHERE id = ?", (new_name, datetime.now(timezone.utc).isoformat(), board_id))
            conn.commit()
            conn.close()
...
def update_board_category(board_id):
    ...
    try:
        with db_lock:
            ...
            cursor.execute("UPDATE boards SET category = ?, updated_date = ? WHERE id = ?", (new_category, datetime.now(timezone.utc).isoformat(), board_id))
            conn.commit()
            conn.close()
...
def get_board_chat_history(board_id):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM board_chats WHERE board_id = ? ORDER BY timestamp ASC", (board_id,))
        messages = [dict(row) for row in cursor.fetchall()]
        conn.close()
...
def move_card_to_board(source_board_id):
    ...
    try:
        with db_lock:
            ...
            cursor.execute("SELECT board_id FROM collaborators WHERE board_id = ? AND user_email = ?", (source_board_id, email))
            if not cursor.fetchone():
                conn.close()
                return jsonify(success=False, message="No tienes permiso en el tablero de origen."), 403
            
            cursor.execute("SELECT board_id FROM collaborators WHERE board_id = ? AND user_email = ?", (target_board_id, email))
            if not cursor.fetchone():
                conn.close()
                return jsonify(success=False, message="No tienes permiso en el tablero de destino."), 403

            cursor.execute("SELECT board_data FROM boards WHERE id = ?", (source_board_id,))
            ...
            cursor.execute("UPDATE boards SET board_data = ?, updated_date = ? WHERE id = ?", 
                           (json.dumps(source_data), datetime.now(timezone.utc).isoformat(), source_board_id))

            cursor.execute("SELECT board_data FROM boards WHERE id = ?", (target_board_id,))
...
def handle_notes():
    if request.method == 'GET':
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM notes WHERE board_id = ? ORDER BY updated_date DESC", (board_id,))
    ...
    if request.method == 'PUT':
        ...
        try:
            with db_lock:
                ...
                cursor = conn.cursor()
                now = datetime.now(timezone.utc).isoformat()
                cursor.execute("UPDATE notes SET content = ?, color = ?, updated_date = ? WHERE id = ? AND user_email = ?", (content, color, now, note_id, email))
                conn.commit()
                if cursor.rowcount == 0:
                    conn.close()
    ...
    if request.method == 'DELETE':
        ...
        try:
            with db_lock:
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("SELECT board_id FROM notes WHERE id = ? AND user_email = ?", (note_id, email))
                note_info = cursor.fetchone()
                if not note_info:
                    conn.close()
                    return jsonify(success=False, message="Nota no encontrada o sin permiso."), 404
                board_id = note_info['board_id']
                cursor.execute("DELETE FROM notes WHERE id = ? AND user_email = ?", (note_id, email))
                conn.commit()
                conn.close()
...
def ask_assistant():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT prompt, knowledge_base FROM assistants WHERE id = ?", (assistant_id,))
...
def handle_list_conversations(data):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM conversations WHERE participants_json LIKE ?", (f'%"{email}"%',))
        ...
        for conv in conversations:
            ...
            cursor.execute("SELECT first_name, last_name FROM users WHERE email = ?", (peer_email,))
...
def handle_start_chat(data):
    ...
    try:
        ...
        with db_lock:
            ...
            cursor.execute("SELECT first_name, last_name FROM users WHERE email = ?", (partner_email,))
            peer_user = cursor.fetchone()
            peer_name = f"{peer_user['first_name']} {peer_user['last_name']}" if peer_user else partner_email
            # --- FIN DE LA CORRECCIÓN ---

            # Obtener el historial de mensajes
            cursor.execute("SELECT * FROM direct_messages WHERE conv_id = ? ORDER BY ts ASC", (conv_id,))
            messages = [dict(row) for row in cursor.fetchall()]
            conn.close()
...
def handle_mark_read(data):
    ...
    try:
        with db_lock:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE direct_messages SET is_read = 1 WHERE conv_id = ? AND receiver_email = ?", (conv_id, user_email))
            conn.commit()
            conn.close()
    except Exception as e:
...
def handle_global_chat_send(data):
    ...
    try:
        with db_lock:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT participants_json FROM conversations WHERE id = ?", (conv_id,))
            row = cursor.fetchone()
            ...
            cursor.execute("UPDATE conversations SET last_ts = ? WHERE id = ?", (now, conv_id))
            conn.commit()
            conn.close()
...
def get_assistants_route():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE a.is_public = 1 OR s.user_email = ?
        """, (email,))
        assistants_raw = [dict(row) for row in cursor.fetchall()]
        for assistant in assistants_raw:
            cursor.execute("SELECT user_email FROM assistant_sharing WHERE assistant_id = ?", (assistant['id'],))
...
def get_pending_notifications():
    ...
    try:
        ...
        cursor.execute("""
            ...
            LEFT JOIN notification_views v ON n.id = v.notification_id AND v.user_email = ?
...
def get_database_background():
    ...
    try:
        ...
        cursor.execute("SELECT background_url FROM database_settings WHERE name = ?", (manager_id,))
